...
export function VoiceWorkspaceSidebar({ active, name, owner, source, status = '草稿', createdAt = '今天 09:20', isNew = false, roleSummary = false, language, verificationHint, onOverview, onSource }: Props) {
  ...
  return <aside className="vws-sidebar" aria-label="声音角色工作台导航">
    <section className="vws-summary"><div className="vws-summary-heading"><span className="vws-summary-mark"><SlidersHorizontal size={17} /></span><h2>{roleSummary ? '当前角色' : '当前草稿'}</h2><span className="vws-badge">{isNew ? '待创建' : status}</span></div><dl>{values.map(([label, value]) => <div key={label}><dt>{label}</dt><dd title={value}>{value}</dd></div>)}</dl>{roleSummary && <div className="vws-summary-tags"><span>{source}</span><span>{status}</span></div>}</section>
    <div className="vws-nav-title">声音角色工作台</div>
    <nav>{NAV.map(({ label, icon: Icon }) => {
      const current = active === label;
      const hint = isNew ? label === '声音来源' ? '保存后可配置' : label === '验证与发布' ? '尚不可用' : label === '版本' ? '尚无版本' : label === '使用记录' ? '暂无' : undefined : label === '验证与发布' ? verificationHint || '尚不可用' : label === '版本' ? '尚无版本' : label === '使用记录' ? '暂无' : undefined;
