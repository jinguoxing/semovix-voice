...
export function VoiceIdentityValidationView({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  const saveValidation = () => {
    try {
      window.localStorage.setItem(`voice-studio-validation:${id}`, JSON.stringify({ candidateId, taskId, publishCandidateId, profileName, profileVersion, updatedAt: new Date().toISOString() }));
      setSaved(true);
  ...
  const freezeProfile = () => {
    ...
    setConfirmationText('');
    setMessage(`Voice Profile ${profileVersion} 已冻结并发布，后续调整需要创建新的版本。`);
  };

  return <div className="voice-validation-page">
    <VoiceWorkspaceSidebar active="验证与发布" name={roleName} owner={ownerName} source="AI 原创设计" status="待冻结" language="中文（普通话）" roleSummary sourceHint="已完成" verificationHint="全部测试完成" onOverview={onBack} onSource={onBack} />
    ...
    <main className="vv-workspace">
      <div className="vv-content">
        ...
        <header className="vv-header"><div><div className="vv-title-row"><h1>验证与发布</h1><span className="vv-freeze-badge">待冻结</span></div><p>根据声音来源执行对应验证策略，检查声音稳定性与使用边界，并冻结发布正式 Voice Profile。</p><div className="vv-title-tags"><span>声音来源：<b>AI 原创设计</b></span><span>验证策略：<b>入围候选稳定性验证</b></span></div><div className="vv-stats"><span>入围候选 <b>3 条</b></span><span>测试场景 <b>5 组</b></span><span>重复生成 <b>3 次 / 候选</b></span><span>测试模型 <b>Qwen3-TTS Base</b></span><span>测试状态 <b>全部完成</b></span></div></div><div className="vv-header-actions"><button type="button" className="vv-secondary" onClick={() => notify('当前没有异常项需要重新运行。')}><RefreshCw size={14} />重新运行异常项</button><button type="button" className="vv-secondary" onClick={saveValidation}><Save size={14} />保存验证结果</button><button type="button" className="vv-primary" onClick={() => setConfirmationOpen(true)}><LockKeyhole size={15} />冻结并发布 {profileVersion}</button></div></header>
        ...
        <div className="vv-main-columns">
          ...
          <aside className="vv-decision-column">
            ...
            <section className="vv-panel vv-profile-panel"><div className="vv-panel-heading"><div><h2>Voice Profile</h2><p>冻结后形成不可变的正式版本。</p></div><span className="vv-freeze-badge">待冻结</span></div><label><span>Profile 名称</span><input value={profileName} onChange={event => setProfileName(event.target.value)} /></label><label><span>版本号</span><input value={profileVersion} onChange={event => setProfileVersion(event.target.value)} /></label><dl><div><dt>生产模型</dt><dd>Qwen3-TTS-12Hz-1.7B-Base</dd></div><div><dt>参考声音</dt><dd>候选 {formatCandidate(publishCandidateId)}</dd></div><div><dt>主要语言</dt><dd>中文（普通话）</dd></div><div><dt>默认表达</dt><dd>专业、可信、克制</dd></div><div><dt>默认语速</dt><dd>中等偏慢</dd></div><div><dt>可见范围</dt><dd>组织内可见</dd></div><div><dt>资产等级</dt><dd>核心声音资产</dd></div></dl></section>
            ...
            <div className="vv-decision-actions"><button type="button" className="vv-quiet" onClick={onBack}>返回声音来源</button><button type="button" className="vv-secondary" onClick={saveValidation}><Save size={13} />保存验证结果</button><button type="button" className="vv-primary" onClick={() => setConfirmationOpen(true)}><LockKeyhole size={14} />冻结并发布 {profileVersion}</button></div>
          </aside>
        </div>
      </div>
    </main>
